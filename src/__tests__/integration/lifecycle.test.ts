...
import { exists, lstat, readFile, readlink, realpath, writeFile } from "node:fs/promises";
...
describe("lifecycle integration", () => {
  ...
  it("adds a repo to the workspace", async () => {
    ...
    const stat = await lstat(defaultBranchPath);
    expect(stat.isSymbolicLink()).toBe(true);

    // full chain: {workspace}/trees/myrepo/main -> ../../../repos/myrepo -> actual path
    expect(await realpath(defaultBranchPath)).toBe(await realpath(repoPath));
  });
  ...
  it("worktree pool entry is a real directory, workspace entry is a symlink", async () => {
    ...
    expect((await lstat(wsEntry)).isSymbolicLink()).toBe(true);
    // Symlink points into the pool
    const target = await readlink(wsEntry);
...
describe("generateClaudeFiles — trees.md", () => {
  ...
  describe("golden format", () => {
    beforeAll(async () => {
      ...
      await writeFile(join(repoA, "CLAUDE.md"), "# Alpha\n");
      await writeFile(join(repoB, "CLAUDE.md"), "# Bravo\n");
    ...
    it("produces exact trees.md with sorted repos and comment lines", async () => {
      const content = await readFile(paths.claudeTreesMd("ws-golden"), "utf8");
      expect(content).toBe(
  ...
  describe("dedup — identical content annotates canonical with deduped slugs", () => {
    ...
    beforeAll(async () => {
      ...
      await writeFile(join(repoPath, "CLAUDE.md"), CONTENT);
    ...
    it("annotates the canonical slug when a duplicate is found", async () => {
      const content = await readFile(paths.claudeTreesMd("ws-dup-same"), "utf8");
      expect(content).toContain("# dup-same/main (also: feature-x)\n");
    });

    it("emits only one @reference for the canonical slug", async () => {
      const lines = (await readFile(paths.claudeTreesMd("ws-dup-same"), "utf8"))
        .split("\n")
  ...
  describe("dedup — different content keeps both entries", () => {
    beforeAll(async () => {
      ...
      await writeFile(join(repoPath, "CLAUDE.md"), "# Main content\n");
    ...
    it("includes both entries when content differs", async () => {
      const content = await readFile(paths.claudeTreesMd("ws-dup-diff"), "utf8");
      expect(content).toContain("# dup-diff/main\n");
      expect(content).toContain("# dup-diff/feature-y\n");
    });

    it("omits the annotation when there are no duplicates", async () => {
      const content = await readFile(paths.claudeTreesMd("ws-dup-diff"), "utf8");
  ...
  describe("dedup — multiple identical slugs appear sorted in annotation", () => {
    ...
    it("lists deduped slugs alphabetically within the annotation", async () => {
      const content = await readFile(paths.claudeTreesMd("ws-multi-dup"), "utf8");
  ...
  describe("dedup — no cross-repo dedup for identical content", () => {
    ...
    beforeAll(async () => {
      ...
      await writeFile(join(repoA, "CLAUDE.md"), CONTENT);
      await writeFile(join(repoB, "CLAUDE.md"), CONTENT);
    ...
    it("includes both repos even when their CLAUDE.md content is identical", async () => {
      const content = await readFile(paths.claudeTreesMd("ws-cross"), "utf8");
  ...
  describe("dedup — feature branch included when default has no CLAUDE.md", () => {
    ...
    it("includes the feature branch when the default branch has no CLAUDE.md", async () => {
      const content = await readFile(paths.claudeTreesMd("ws-no-main"), "utf8");
      expect(content).toContain("@../trees/no-main-claude/feature-only/CLAUDE.md");
    });

    it("omits the default branch entry when it has no CLAUDE.md", async () => {
      const content = await readFile(paths.claudeTreesMd("ws-no-main"), "utf8");
  ...
  describe("dedup — default branch wins over alphabetically-earlier slug", () => {
    ...
    it("uses main as canonical even when another slug is alphabetically earlier", async () => {
      const content = await readFile(paths.claudeTreesMd("ws-default-wins"), "utf8");
